...
const openSource = {
  githubConvertedToken: "process.env.REACT_APP_GIT_TOKEN",
  githubUserName: "mrXavi3r",
};
...
fetch(baseUrl, {
  ...
  .then((txt) => {
    const data = JSON.parse(txt);
    console.log(data, openSource.githubConvertedToken);
...
fetch(baseUrl, {
  ...
  .catch((error) => console.log("Error occured in pinned projects 2", error));
